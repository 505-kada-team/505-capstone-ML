# NOTE: this script is for LOCAL / OFFLINE training only.
# It is intentionally NOT part of the Vercel deployment (see .vercelignore).
# Run it with `pip install -r requirements-train.txt`, then copy the
# resulting saved_models/model.json + metadata.json into the deploy repo.
...
def train_model():
    ...
    # Use a lightweight RandomForestRegressor for faster, dependency-light
    # inference compared with XGBoost. This model is serialized with joblib.
